...
# a patient
# It also does this for various distributions of Training and test-
# ing data whose selection depenedn on the seed
...
i = 1
prediction_accuracies = vector(length = nreps)
for(seed in seeds){
...
# Create ids that index where what is from 
train_id_indeces <- rep(train_ids, each = 3)
test_id_indeces <- rep(test_ids, each = 3)
...
plot(seq_len(length(test_prediction)), as.numeric(test_prediction) - 1,
